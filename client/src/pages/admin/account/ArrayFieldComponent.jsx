...
const ArrayFieldComponent = ({ initialValues = [], type = 'text', onChange }) => {
  const [items, setItems] = useState(
    initialValues.map(item => (type === 'date' && !(item instanceof Date) ? new Date(item) : item))
  )
  ...
  useEffect(() => {
    ...
    if (type === 'date') {
      ...
      onChange(items.map(item => item.toLocaleDateString()))
    } else {
      onChange(items)
    }
  }, [items, onChange, type])
          ...
          startAdornment: (
            <Box sx={{ display: 'flex' }}>
              {items.map((item, index) => (
                <Chip key={index} label={item} onDelete={handleDelete(item)} size='small' />
              ))}
            </Box>
          )
        }}
      ...
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <Stack direction='row' spacing={1} alignItems='center' flexWrap='wrap'>
          ...
          {items.map((date, index) => (
            <Chip key={index} label={date.toLocaleDateString()} onDelete={handleDelete(date)} />
          ))}
        </Stack>
